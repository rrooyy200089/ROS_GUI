#!/bin/bash

# ( # 這裡開始背景執行

export DISPLAY=$(echo $DISPLAY)
export XAUTHORITY=/home/$USER_NAME/.Xauthority

USER=$(whoami)
SESSION_ID=$(loginctl | grep $USER | awk '{print $1}')
# SESSION_ID=2

if [[ -z "$SESSION_ID" ]]; then
    echo "無法獲取 Session ID，可能是錯誤的用戶。" >> /home/roy/screen_log.txt
    exit 1
fi

echo "等待畫面解鎖..." >> /home/roy/screen_log.txt
# (
while true; do
    SESSION_STATUS=$(loginctl show-session $SESSION_ID -p LockedHint | awk -F= '{print $2}')
    
    if [[ "$SESSION_STATUS" == "no" ]]; then
        break
    fi
    echo "lj svjvjvjnsdvlob $(date)" >> /home/roy/screen_log.txt
    sleep 1  # 每秒檢查一次，避免 CPU 過度使用
done

# 當畫面解鎖時，紀錄當前時間

echo "cccccccccccc1" >> /home/roy/screen_log.txt
UNLOCK_TIME=$(date +"%Y-%m-%d %H:%M:%S")
echo "畫面已解鎖，時間：$UNLOCK_TIME" >> /home/roy/screen_log.txt

source ~/.bashrc

# 

# roscore
# sleep 5
# roslaunch robot_control gui.launch
gnome-terminal -- bash -c '/home/roy/project/gui_ws/src/robot_control/Script/restart_script_stage_1.sh'


# ) &
